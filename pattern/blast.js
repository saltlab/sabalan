var smith_waterman = require('./smith-waterman');
...
function extractQueryWords(querySequence, dbSequences, k) {
    for (var i = 0; i <= querySequence.length - k; i ++) {
        //var queryWord = "";
        var queryWord = [];
        for (var j = 0; j < k; j ++) {
            //queryWord += querySequence[i + j].name;
...
function expandMatch(querySeq, dbSeq, querySeqIndex, dbSeqIndex, k) {
    ...
    var queryEndIndex = querySeqIndex + wordLength - 1;
    var dbBeginIndex = dbSeqIndex;
    var dbEndIndex = dbSeqIndex + wordLength - 1;
